...
int Board::GetKeyValue(int row, int col, int i)
{
	const int stepX = X[i];
	const int stepY = Y[i];
	int key = (chess_map[row - stepX * 4][col - stepY * 4].color) ^ (chess_map[row - stepX * 3][col - stepY * 3].color << 2) ^ (chess_map[row - stepX * 2][col - stepY * 2].color << 4) ^ (chess_map[row - stepX * 1][col - stepY * 1].color << 6) ^ (chess_map[row + stepX * 1][col + stepY * 1].color << 8) ^ (chess_map[row + stepX * 2][col + stepY * 2].color << 10) ^ (chess_map[row + stepX * 3][col + stepY * 3].color << 12) ^ (chess_map[row + stepX * 4][col + stepY * 4].color << 14);
	return key;
}
// 判断key棋型，填充棋型表
...
Point AI::RootSearch(int depth, int alpha, int beta, MoveLine *pline)
{
	...
	for (int i = 0; i < rootMoveCount; i++)
	{
		...
		if (!mustLosePoint[pos.row][pos.col])
		{
			...
			if (value == -10000)
				mustLosePoint[pos.row][pos.col] = true;
...
int AI::Evaluate()
{
	...
	// 2.对方存在两个成五点，必败
	// 3.对方不能成五，己方存在活四
	if (playerType[FIVE] >= 1)
		return 10000;
	if (enemyType[FIVE] >= 2)
		return -10000;
	if (enemyType[FIVE] == 0 && playerType[LIVEFOUR] >= 1)
		return 10000;
